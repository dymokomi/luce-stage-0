...
void digest_block(const Byte *block, U32 state[8]) {
  ...
  for (int i = 16; i < 64; ++i) {
    const U32 first = rotate_right(words[i - 15], 7) ^ rotate_right(words[i - 15], 18) ^
                      (words[i - 15] >> 3);
    const U32 second = rotate_right(words[i - 2], 17) ^ rotate_right(words[i - 2], 19) ^
                       (words[i - 2] >> 10);
  ...
  U32 h = state[7];
  for (int i = 0; i < 64; ++i) {
    const U32 sum_one = rotate_right(e, 6) ^ rotate_right(e, 11) ^ rotate_right(e, 25);
    const U32 choice  = (e & f) ^ ((~e) & g);
    const U32 temporary_one = h + sum_one + choice + constants[i] + words[i];
    const U32 sum_zero = rotate_right(a, 2) ^ rotate_right(a, 13) ^ rotate_right(a, 22);
...
String content_digest(const Bytes &bytes) {
  ...
  for (int i = 0; i < 8; ++i) {
    final_blocks[final_size - 1 - i] = static_cast<Byte>((bit_size >> (i * 8)) & 0xffu);
  }
  ...
  for (int word = 0; word < 8; ++word) {
    snprintf(text + word * 8, 9, "%08x", static_cast<unsigned int>(state[word]));
  }
  text[64] = '\0';
  return String(text);
}

bool entry_bytes(const Store &store, const ProjectionEntry &entry, Bytes *bytes,
                 U64 *revision) {
  Texel      texel;
  OutputPort output;
  if (bytes == 0 || revision == 0 || !store.get(entry.texel(), &texel) ||
      !texel.get_output(entry.output().c_str(), &output) || output.type() != entry.type() ||
      !output.has_source() || output.source().type() != entry.type()) {
    return false;
  }
...
bool current_revision(const Store &store, const ProjectionRecord &record) {
  ...
  return store.get(record.entry().texel(), &texel) &&
         ...
         output.type() == record.entry().type() && output.has_source() &&
...
bool open_root(const String &directory, int *root) {
  ...
  const int opened =
      open(directory.c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
  if (opened < 0) {
    return false;
  }
  *root = opened;
...
bool open_parent(int root, const String &filename, bool create, int *parent, String *leaf) {
  ...
  while (slash != String::npos) {
    ...
    int          child =
        openat(directory, part.c_str(), O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
...
bool read_file(int root, const String &filename, Bytes *bytes) {
  ...
  const int file = openat(parent, leaf.c_str(), O_RDONLY | O_NOFOLLOW | O_CLOEXEC);
  close(parent);
  if (file < 0) {
    return false;
  }
  ...
  struct stat status;
  if (fstat(file, &status) != 0 || !S_ISREG(status.st_mode) || status.st_nlink != 1 ||
      status.st_size < 0 || static_cast<U64>(status.st_size) > static_cast<U64>(SIZE_MAX)) {
    close(file);
    return false;
  }
  ...
  while (offset < loaded.size()) {
    const ssize_t count = ::read(file, loaded.data() + offset, loaded.size() - offset);
...
bool write_file(int root, const String &filename, const Bytes &bytes) {
  ...
  const int file =
      openat(parent, leaf.c_str(), O_WRONLY | O_CREAT | O_NOFOLLOW | O_CLOEXEC, 0600);
  close(parent);
  if (file < 0) {
    return false;
  }
...
bool check_tree(int directory, const String &prefix, const ProjectionManifest &manifest) {
  ...
  while ((item = readdir(scan)) != 0) {
    ...
    const String path = prefix.empty() ? String(item->d_name) : prefix + "/" + item->d_name;
    struct stat  status;
    if (fstatat(directory, item->d_name, &status, AT_SYMLINK_NOFOLLOW) != 0) {
      valid = false;
      break;
    }
    if (S_ISREG(status.st_mode)) {
      ...
      const int child =
          openat(directory, item->d_name, O_RDONLY | O_DIRECTORY | O_NOFOLLOW | O_CLOEXEC);
...
bool put_change(Transaction *transaction, const ProjectionRecord &record,
  ...
  Texel      texel;
  OutputPort output;
  if (transaction == 0 || !transaction->get(record.entry().texel(), &texel) ||
      !texel.get_output(record.entry().output().c_str(), &output) ||
      output.type() != record.entry().type() || output.revision() != record.revision()) {
    return false;
  }
  ...
  return output.set_source(value) && texel.put_output(output) && transaction->put(texel);
}

} // namespace

ProjectionRecord::ProjectionRecord() : source_revision(0) {}

ProjectionRecord::ProjectionRecord(const ProjectionEntry &entry, U64 revision,
                                   const String &digest)
    : projection_entry(entry), source_revision(revision), content_digest(digest) {}
...
FileProjection::FileProjection() : exported(false) {}

bool FileProjection::export_from(const Store &store, const ProjectionManifest &manifest,
                                 const char *directory) {
  if (!store.is_open() || manifest.size() == 0 || directory == 0 || directory[0] == '\0') {
    return false;
  }
  ...
  for (Size i = 0; i < manifest.size(); ++i) {
    ...
    Bytes           bytes;
    U64             revision = 0;
    if (!manifest.at(i, &entry) || !entry_bytes(store, entry, &bytes, &revision) ||
        !write_file(root, entry.filename(), bytes)) {
      close(root);
      return false;
    }
    captured.push_back(ProjectionRecord(entry, revision, content_digest(bytes)));
  }
  close(root);
...
bool FileProjection::import_changes(Store *store) {
  ...
  int root = -1;
  if (!open_root(root_directory, &root) || !check_tree(root, "", projection_manifest)) {
    if (root >= 0) {
      close(root);
    }
    return false;
  ...
  for (Size i = 0; i < changes.size(); ++i) {
    if (!put_change(&transaction, records[changes[i].record_index], changes[i].bytes)) {
      transaction.abort();
      return false;
    }
  }
  ...
  for (Size i = 0; i < changes.size(); ++i) {
    ...
    records[index] =
        ProjectionRecord(records[index].entry(), output.revision(), changes[i].digest);
  }
  return true;
}
