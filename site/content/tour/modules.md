...
```output
luce: compile failed
main.luc:1:1: there is no standard module std.nope; the standard library is std.math, std.files, std.strings, std.paths, std.os, std.zip [luce.import.standard]
    import std.nope
    ^~~~~~~~~~~~~~~
```

There are five standard modules today: [`math`](/std/math/),
[`strings`](/std/strings/), [`files`](/std/files/),
[`paths`](/std/paths/) and [`os`](/std/os/). They are ordinary Luce
