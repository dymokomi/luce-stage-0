...
bool write_body(Volume *volume, int arena, const Bytes &body) {
  ...
  for (U64 i = 0; i < needed; ++i) {
    ...
    const Size chunk =
        remaining < static_cast<Size>(PAGE_SIZE) ? remaining : static_cast<Size>(PAGE_SIZE);
...
bool write_descriptor(Volume *volume, int arena, U64 generation, const Bytes &body) {
  ...
  put_u64(descriptor + DESCRIPTOR_CHECKSUM_OFFSET, body_checksum(body.data(), body.size()));
...
bool Transaction::referenced(const TexelId &id) const {
  for (TexelTable::const_iterator found = texels.begin(); found != texels.end(); ++found) {
    ...
    for (Size i = 0; i < texel.output_size(); ++i) {
      ...
      if (texel.output_at(i, &output) && output.has_source() &&
          output.source().type() == VALUE_TEXEL && output.source().texel().equals(id)) {
        return true;
      }
    }
  }
