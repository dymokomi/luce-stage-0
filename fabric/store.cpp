...
const Byte DESCRIPTOR_MAGIC[DESCRIPTOR_MAGIC_SIZE] = {
  'L', 'U', 'S', 'T', 'O', 'R', 'E', '\0'
};
...
bool write_descriptor(Volume* volume, int arena, U64 generation,
                      const Bytes& body)
{
  ...
  put_u64(descriptor + DESCRIPTOR_BODY_SIZE_OFFSET,
          static_cast<U64>(body.size()));
...
{
  if (body == 0 || byte_size == 0 ||
      byte_size > static_cast<U64>(SIZE_MAX)) {
    return false;
  }
  const U64 needed =
      (byte_size + static_cast<U64>(PAGE_SIZE) - 1) / PAGE_SIZE;
  if (needed == 0 || needed > arena_pages(volume)) {
    return false;
  }
  ...
  for (U64 i = 0; i < needed; ++i) {
    ...
    const Size chunk =
        remaining < static_cast<U64>(PAGE_SIZE)
...
bool load_snapshot(Volume* volume, int arena, LoadedSnapshot* loaded)
{
  Byte descriptor[PAGE_SIZE];
  if (loaded == 0 ||
      !volume->read(static_cast<U64>(arena), descriptor) ||
      memcmp(descriptor, DESCRIPTOR_MAGIC, sizeof(DESCRIPTOR_MAGIC)) != 0 ||
      get_u32(descriptor + DESCRIPTOR_VERSION_OFFSET) != STORE_VERSION ||
      get_u32(descriptor + 12) != 0 ||
      !descriptor_tail_clear(descriptor)) {
    return false;
  }

  const U64 generation =
      get_u64(descriptor + DESCRIPTOR_GENERATION_OFFSET);
  const U64 byte_size =
      get_u64(descriptor + DESCRIPTOR_BODY_SIZE_OFFSET);
  const U64 checksum =
      get_u64(descriptor + DESCRIPTOR_CHECKSUM_OFFSET);
  if (generation == 0) {
    return false;
  }
                        ...
                        Texels* texels, BlobRecords* blobs)
{
  texels->clear();
  blobs->clear();
  for (TexelTable::const_iterator found = table.begin();
       found != table.end(); ++found) {
    texels->push_back(found->second);
  }
  for (BlobTable::const_iterator found = blob_table.begin();
       found != blob_table.end(); ++found) {
    blobs->push_back(found->second);
  }
}

void snapshot_to_tables(const Texels& texels, const BlobRecords& blobs,
                        TexelTable* table, BlobTable* blob_table)
{
  table->clear();
  blob_table->clear();
  ...
  for (int word = 0; word < 4; ++word) {
    ...
    for (Size i = 0; i < bytes.size(); ++i) {
      hash ^= static_cast<U64>(bytes[i]) +
              (static_cast<U64>(word + 1) << 8);
      hash *= UINT64_C(1099511628211);
      hash ^= hash >> 29;
    }
    hash ^= static_cast<U64>(word + 1) * UINT64_C(0x9e3779b97f4a7c15);
    for (int byte = 0; byte < 8; ++byte) {
      identifier[word * 8 + byte] =
          static_cast<Byte>((hash >> (byte * 8)) & 0xffu);
...
{
  for (TexelTable::const_iterator found = texels.begin();
       found != texels.end(); ++found) {
    const Texel& texel = found->second;
    if (texel.has_content() &&
        texel.content().type() == VALUE_TEXEL &&
...
{
  if (!active || id.is_unset() || texels.find(id) == texels.end() ||
      referenced(id)) {
    return false;
  }
  texels.erase(id);
...
{
  if (!active || input_name == 0 || output_name == 0 ||
      target.is_unset() || source.is_unset()) {
    return false;
  }
  ...
  if (!changed.get_input(input_name, &input) ||
      ...
      !input.bind(Fiber(source, output_name)) ||
...
bool Transaction::get_blob(const BlobRef& reference, Bytes* bytes) const
{
  ...
  if (found == blobs.end() ||
      found->second.reference.size() != reference.size()) {
    return false;
  }
...
bool Store::create(Volume* store_volume)
{
  ...
  if (!store_volume->write(0, empty_descriptor) ||
      !store_volume->write(1, empty_descriptor) ||
      !store_volume->flush()) {
    return false;
  }
...
bool Store::open(Volume* store_volume)
{
  ...
  snapshot_to_tables(newest->texels, newest->blobs,
                     &loaded_texels, &loaded_blobs);
  volume = store_volume;
...
bool Store::commit(Transaction* transaction)
{
  ...
  tables_to_snapshot(transaction->texels, transaction->blobs,
                     &snapshot_texels, &snapshot_blobs);
  Bytes body;
