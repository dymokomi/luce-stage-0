...
#include "fabric/output_port.h"
#include "fabric/store.h"
#include "fabric/texel.h"
#include "projection/file_projection.h"
#include "projection/projection_manifest.h"
...
static bool write_bytes(const char* path, const Bytes& bytes)
{
  ...
  const Size written =
      bytes.empty() ? 0 : fwrite(bytes.data(), 1, bytes.size(), file);
...
static bool output_value(const Store& store, const TexelId& id,
                         const char* name, OutputPort* output)
{
  Texel texel;
  return store.get(id, &texel) && texel.id().equals(id) &&
         texel.get_output(name, output);
}
...
static void test_projection()
{
...

  {
    ...
    CHECK(!rejected.put(
        ProjectionEntry(text_id, "text", VALUE_TEXT, "/absolute")));
    CHECK(!rejected.put(
        ProjectionEntry(text_id, "text", VALUE_TEXT, "../escape")));
    CHECK(!rejected.put(
        ProjectionEntry(text_id, "text", VALUE_TEXT, "a/../../escape")));
    CHECK(!rejected.put(
        ProjectionEntry(text_id, "text", VALUE_TEXT, "a\\escape")));
    CHECK(!rejected.put(
        ProjectionEntry(text_id, "text", VALUE_BYTES, "bytes.bin")));

    ProjectionManifest manifest;
    CHECK(manifest.put(
        ProjectionEntry(text_id, "text", VALUE_TEXT, "notes/message.txt")));
    CHECK(manifest.put(
        ProjectionEntry(blob_id, "blob", VALUE_BLOB, "assets/data.bin")));
    CHECK(!manifest.put(
        ProjectionEntry(blob_id, "other", VALUE_BLOB, "assets/data.bin")));
    CHECK(!manifest.put(
        ProjectionEntry(text_id, "text", VALUE_TEXT, "notes/other.txt")));
...

  {
    ...
    CHECK(reopened.get_blob(persisted_blob.source().blob(),
                            &persisted_bytes));
