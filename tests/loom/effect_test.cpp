...
public:
  RecordingExecutor() : calls(0), actions(0), has_request(false) {
    memset(request, 0, sizeof(request));
  }

  bool perform(const EffectIntent &intent, EffectObservation *observation) override {
    ++calls;
    if (!has_request) {
...
static bool put_effect(Store *store, const Texel &effect, const Texel *capability) {
  ...
  if (capability != 0 &&
      (!transaction.put(*capability) ||
       !transaction.connect(effect.id(), "capability", capability->id(), "capability"))) {
    transaction.abort();
    return false;
  }
...
static void test_capability_and_effect_encoding() {
  ...
  CHECK(encode_effect_observation(original_observation, &encoded_observation));
  EffectObservation decoded_observation;
  CHECK(decode_effect_observation(encoded_observation, &decoded_observation));
  ...
  CHECK(!decode_effect_observation(Value(malformed), &decoded_observation));
  malformed = encoded_observation.bytes();
  malformed.resize(malformed.size() - 1);
  CHECK(!decode_effect_observation(Value(malformed), &decoded_observation));
}
...
static void test_boundary_authority_and_durable_receipt() {
  ...
  Texel no_capability;
  CHECK(make_effect_texel(new_id(), intent(10, "/device/one"), &no_capability));
  ...
  CHECK(make_effect_texel(new_id(), intent(50, "/device/one"), &wrong_effect));
  ...
  CHECK(effect_observation_id(correct_intent.request_id(), &observation_id));
  ...
  CHECK(authority.encode(&authority_value));
  Texel      authority_texel(new_id());
  OutputPort authority_output("authority", VALUE_BYTES);
  CHECK(authority_output.set_source(authority_value));
  CHECK(authority_texel.put_output(authority_output));
  Transaction authority_transaction;
  CHECK(store.begin(&authority_transaction));
  CHECK(authority_transaction.put(authority_texel));
  CHECK(authority_transaction.commit());

  Store reopened;
  CHECK(reopened.open(&volume));
  Texel      restored_authority_texel;
  OutputPort restored_authority_output;
  Authority  restored_authority;
  CHECK(reopened.get(authority_texel.id(), &restored_authority_texel));
  CHECK(restored_authority_texel.get_output("authority", &restored_authority_output));
  CHECK(restored_authority.restore(restored_authority_output.source()));
  CHECK(restored_authority.size() == authority.size());

  Spool          reopened_spool(&reopened, &evaluators);
  EffectBoundary reopened_boundary(&reopened, &reopened_spool, &restored_authority,
                                   &executors);
  ...
  Texel restarted_effect;
  CHECK(make_effect_texel(new_id(), intent(120, "/device/one"), &restarted_effect));
  Transaction restarted_transaction;
  CHECK(reopened.begin(&restarted_transaction));
  CHECK(restarted_transaction.put(restarted_effect));
  CHECK(restarted_transaction.connect(restarted_effect.id(), "capability",
                                      correct_capability.id(), "capability"));
  CHECK(restarted_transaction.commit());
  result = reopened_boundary.perform(restarted_effect.id());
  CHECK(result.code() == EFFECT_PERFORMED);
  CHECK(executor.calls == 2);
  CHECK(executor.actions == 2);
}
