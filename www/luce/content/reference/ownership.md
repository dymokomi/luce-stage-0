...
| Topic | Rules |
|---|---|
| Creating and dropping values | [S1–S7](#s1) |
| Aliases | [S8–S10](#s8) |
| Function calls | [S11–S15](#s11) |
| Returns | [S16–S19](#s16) |
| Containers | [S20–S23](#s20) |
| Structs | [S24–S28](#s24) |
| `give` and `copy` | [S29–S32](#s29) |
| Program boundaries | [S33–S35](#s33) |
| Scope clarifications | [S36–S40](#s36) |
| Null and absence | [S41–S43](#s41) |
| Entry arguments and return shapes | [S44–S45](#s44) |
| File-scope constant containers | [S46](#s46) |

## Vocabulary

**container object** — one of `list`, `map`, `array`, `builder`. It
owns the elements it stores. `copy` can duplicate it when its complete
type graph has no resource.

**resource** — a scope-owned heap handle for a `file` or `task`. It
moves with `give` or `return` and releases with `free` or scope end. It
cannot be copied.
...
**fresh** — an owned expression that has not been named: `new ...`, a
literal, an admitted slice, a call result, `s.split(x)`, `m.keys()`,
`pop()`, `files.open(path)`, or `spawn f()`.
...
`use_after_free` is deterministic and names the faulting line. It is
reported in every build mode.
...
`give` inside a branch poisons the name after the `if`, regardless of
which branch executes. For a resource-free graph, use `copy`; a resource
graph must be created inside the branch or moved once before it.
...
`T?` does not change the ownership rules. Error propagation follows
[S4](#s4), releasing the scopes it leaves.
