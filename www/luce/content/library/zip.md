...
| Method | Behavior |
|---|---|
| `writer.add(name: str, data: list[u8], compress: bool = false) -> !` | adds one entry; when `compress` is true, keeps DEFLATE only if it is smaller |
...
| Signature | Result |
|---|---|
| `zip.inflate(data: list[u8]) -> list[u8]!` | expands stored, fixed-Huffman, or dynamic-Huffman raw DEFLATE blocks |
...
| Signature | Result |
|---|---|
| `zip.read(path: str) -> list[u8]!` | reads an archive's bytes with `std.files.read_bytes` |
| `zip.write(path: str, archive: list[u8]) -> !` | writes archive bytes with `std.files.write_bytes` |
