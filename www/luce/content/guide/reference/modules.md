...
```output
luce: compile failed
main.luc:1:1: there is no standard module std.nope; the standard library is std.math, std.files, std.strings, std.lists, std.paths, std.os, std.term, std.zip, std.json, std.gpu, std.ui, std.network [luce.import.standard]
    import std.nope
    ^~~~~~~~~~~~~~~
```
