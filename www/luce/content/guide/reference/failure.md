...
| `invalid_utf8` | C text is not valid UTF-8 | An extern declared `-> str`, or `c.cstring_at`, met text that is not valid UTF-8. A `str` is valid UTF-8 by contract; arbitrary bytes are read with `c.bytes_at`. |
