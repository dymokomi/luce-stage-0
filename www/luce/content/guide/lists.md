...
`find` returns `long?`: use `else` when you want a fallback such as `-1`.
That keeps “not found” separate from a valid index and matches the other
optional lookups in the language.
