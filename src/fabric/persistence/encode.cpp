...
public:
  ...
  bool read(Byte *output, Size count) {
    if ((data == 0 && size != 0) || count > size - offset || (output == 0 && count != 0)) {
      return false;
    }
    if (count != 0) {
  ...
  bool string(String *text) {
    ...
    text->assign(reinterpret_cast<const char *>(data + offset), static_cast<Size>(length));
...
bool decode_texel_body(const Byte *data, Size size, Texel *output) {
  ...
  for (U32 i = 0; i < input_count; ++i) {
    ...
    if (!reader.string(&name) || name.empty() || texel.has_input(name.c_str()) ||
        !reader.u8(&type) || type < VALUE_BOOL || type > VALUE_BLOB || !reader.u8(&bound) ||
        bound > 1) {
      return false;
    }
...
bool validate_snapshot(const Texels &texels, const BlobRecords &blobs) {
  ...
  for (Size i = 0; i < texels.size(); ++i) {
    ...
    for (Size input_index = 0; input_index < texel.input_size(); ++input_index) {
      ...
      TexelIndexes::const_iterator source = texel_indexes.find(input.binding().source());
      if (source == texel_indexes.end()) {
        return false;
      }
      OutputPort output;
      if (!texels[source->second].get_output(input.binding().output().c_str(), &output) ||
    ...
    for (Size output_index = 0; output_index < texel.output_size(); ++output_index) {
      ...
      if (!texel.output_at(output_index, &output) ||
          (output.has_source() &&
           !value_references_valid(output.source(), texel_indexes, blob_indexes, blobs))) {
        return false;
      }
    }
  }
...
bool decode_snapshot(const Byte *data, Size size, Texels *texels, BlobRecords *blobs) {
  ...
  for (U32 i = 0; i < blob_count; ++i) {
    ...
    blob.bytes.assign(reader.current(), reader.current() + static_cast<Size>(byte_size));
