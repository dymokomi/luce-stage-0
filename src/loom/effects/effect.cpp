...
bool same_request(const Byte *left, const Byte *right) {
  return left != 0 && right != 0 && memcmp(left, right, EffectIntent::REQUEST_ID_SIZE) == 0;
}
...
bool decode_effect_intent(const Value &value, EffectIntent *intent) {
  ...
  const Size fixed_size = EFFECT_MAGIC_SIZE + 4 + EffectIntent::REQUEST_ID_SIZE + 4 + 4 + 4;
...
EffectBoundaryResult EffectBoundary::perform(const TexelId &effect) {
  ...
  if (!authority->verify(capability, intent.operation().c_str(), intent.target().c_str())) {
