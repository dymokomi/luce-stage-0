...
Shell::Shell(Store *fabric_store)
    : store(fabric_store), spool(fabric_store, &registry), has_focus(false), focus_index(0),
      ready(false) {
...
bool Shell::add(const TexelId &view, const char *accessibility_label) {
    ...
    Texel texel;
    if (!store->get(view, &texel) || (texel.evaluator() != PROSE_VIEW_EVALUATOR &&
...
bool Shell::interface(const TexelId &view, String *text) {
    ...
    if (!spool.demand(view, VIEW_INTERFACE_OUTPUT, &outcome) ||
        outcome.status() != VALUE_AVAILABLE || outcome.value().type() != VALUE_TEXT) {
        return false;
    }
...
bool Shell::edit(const TexelId &source, const char *output_name, const char *text) {
    if (!is_ready() || !has_focus || source.is_unset() || output_name == 0 ||
        output_name[0] == '\0' || text == 0) {
        return false;
    }
    ...
    if (!transaction.get(source, &texel) || !texel.evaluator().empty() ||
        !texel.get_output(output_name, &output) || output.type() != VALUE_TEXT ||
        !output.set_source(Value(text)) || !texel.put_output(output) ||
