...
//!
//! **Bytes are a `list(long)`, one byte to an element**, because the
...
test "zip: CRC-32 answers the published check values" {
    try agreeOk(
        ...
        \\
        \\    # Bytes no string could hold: three NULs, four 0xFFs, and
        \\    # every byte there is, in order.
        \\    var nuls: list(long) = [0, 0, 0]
        \\    assert(zip.crc32(nuls) == 4282505490)
        \\    var ones: list(long) = [255, 255, 255, 255]
...
test "zip: bytes and text are inverse over anything a string can hold" {
    try agreeOk(
        ...
        \\    let plain_again = try zip.text(zip.bytes(plain))
        \\    assert(plain_again == plain)
        \\    let wide = "héllo — λ"
        \\    let wide_again = try zip.text(zip.bytes(wide))
        \\    assert(wide_again == wide)
        \\    let nothing = try zip.text(zip.bytes(""))
        \\    assert(nothing == "")
        \\
        \\    # A NUL is a character like any other: one byte, and it
        \\    # survives both ways.
        \\    var nul: list(long) = [104, 0, 105]
        \\    let held = try zip.text(nul)
        \\    assert(len(held) == 3)
...
test "zip: bytes that are not text say so rather than trapping" {
    try agreeRaises(
        \\import std.zip
        \\
        \\func main() -> !:
        \\    var loose: list(long) = [111, 107, 128]
        \\    let held = try zip.text(loose)
        \\
    , "zip: those bytes are not text (byte 2)");

    try agreeRaises(
        \\import std.zip
        \\
        \\func main() -> !:
        \\    var cut: list(long) = [99, 97, 102, 195]
        \\    let held = try zip.text(cut)
        \\
    , "zip: those bytes are not text (they end mid-character)");

    try agreeRaises(
        \\import std.zip
        \\
        \\func main() -> !:
        \\    var overlong: list(long) = [192, 175]
        \\    let held = try zip.text(overlong)
        \\
    , "zip: those bytes are not text (an overlong character at byte 0)");

    try agreeRaises(
        \\import std.zip
        \\
        \\func main() -> !:
        \\    var surrogate: list(long) = [237, 160, 128]
        \\    let held = try zip.text(surrogate)
        \\
    , "zip: those bytes are not text (a surrogate at byte 0)");
}
...
test "zip: an archive this module wrote is one it can read" {
    try agreeOk(
        ...
        \\    # text, every byte there is, and one long enough to have
        \\    # a structure.
        \\    var nothing: list(long) = []
        \\    var every: list(long) = []
        \\    for value in range(0, 256):
        \\        every.append(value)
        \\    var large: list(long) = []
        \\    for step in range(0, 5000):
        \\        large.append(step % 251)
        \\
        ...
        \\    let said = try zip.text(greeting)
...
test "zip: deflate and inflate are inverse, on text and on raw bytes" {
    try agreeOk(
        \\import std.zip
        \\
        \\func same(left: list(long), right: list(long)) -> bool:
        ...
        \\
        \\    # Every byte, which compresses to nothing worth having.
        \\    var every: list(long) = []
        \\    for value in range(0, 256):
        \\        every.append(value)
        ...
        \\
        \\    # One byte repeated far enough to need a long match copying
...
test "zip: what Info-ZIP stored, std.zip reads" {
    try agreeOk(
        ...
        \\    let first_text = try zip.text(first)
        ...
        \\    let second_text = try zip.text(second)
...
test "zip: a compressed stream that is not one says so" {
    try agreeRaises(
        ...
        \\    var nonsense: list(long) = [7, 0, 0, 0]
    ...
    try agreeRaises(
        ...
        \\    var wrong: list(long) = [1, 4, 0, 0, 0, 65, 66, 67, 68]
    ...
    try agreeRaises(
        ...
        \\    var short: list(long) = [3]
        \\    let back = try zip.inflate(short)
        \\
...
test "zip: a signature inside a comment is not the end record" {
    ...
    try agreeOk(
        ...
        \\    archive[len(archive) - 2] = len(inner) & 0xFF
        \\    archive[len(archive) - 1] = len(inner) >> 8 & 0xFF
        \\    for index in range(0, len(inner)):
        \\        archive.append(inner[index])
        \\    let found = try zip.entries(archive)
        \\    assert(len(found) == 2)
