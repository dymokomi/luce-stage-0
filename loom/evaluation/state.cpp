#include "loom/evaluation/state.h"
...
bool temporal_shape(const Texel &texel, InputPort *next, OutputPort *value) {
  if (!temporal_evaluator(texel.evaluator()) || texel.input_size() != 1 ||
      texel.output_size() != 1 || !texel.get_input("next", next) ||
      !texel.get_output("value", value)) {
    return false;
  }
  return next->type() == value->type() && value->has_source() &&
         value->source().type() == value->type();
}

bool create_temporal(const TexelId &id, const char *evaluator, ValueType type,
                     const Value &initial, Texel *texel) {
  if (texel == 0 || id.is_unset() || type == VALUE_NONE || initial.type() != type) {
    return false;
  }

  Texel      created(id);
  OutputPort value("value", type);
  if (!created.set_evaluator(evaluator) || !created.put_input(InputPort("next", type)) ||
      !value.set_source(initial) || !created.put_output(value)) {
    return false;
  }
  *texel = created;
  return true;
...
bool TemporalRuntime::advance(Store *store, Spool *spool, const TexelId &id,
  ...
  if (!transaction.get(id, &changed) ||
      ...
      !current_value.set_source(outcome.value()) || !changed.put_output(current_value) ||
