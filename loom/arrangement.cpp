...
enum {
  HEADER_SIZE = 8,
  ENTRY_FIXED_SIZE = 1 + TexelId::SIZE,
  VERSION = 1
};
...
{
  return static_cast<Size>(data[0]) |
         (static_cast<Size>(data[1]) << 8);
}
...
bool write(Texel* arrangement, const ArrangementEntries& entries)
{
  ...
  Bytes bytes;
  return encode(entries, &bytes) &&
         arrangement->set_content(Value(bytes));
}

bool find_entry(const ArrangementEntries& entries, const char* name,
                Size* index)
{
  if (name == 0 || index == 0) {
    return false;
...
bool inspect_arrangement(const Texel& arrangement,
                         ArrangementEntries* entries)
{
  if (!arrangement.has_content() ||
      arrangement.content().type() != VALUE_BYTES) {
    return false;
  }
...
bool arrangement_add(Texel* arrangement, const char* name,
                     const TexelId& texel)
{
...
bool arrangement_rename(Texel* arrangement, const char* name,
...
bool arrangement_reorder(Texel* arrangement, const char* name, Size index)
{
  ...
  Size current = 0;
  if (!inspect_arrangement(*arrangement, &entries) ||
      index >= entries.size() || !find_entry(entries, name, &current)) {
    return false;
  }
