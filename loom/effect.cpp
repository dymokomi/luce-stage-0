...
const Byte INTENT_MAGIC[EFFECT_MAGIC_SIZE] = {
  'L', 'U', 'E', 'F', 'I', 'N', 'T', '\0'
};
const Byte OBSERVATION_MAGIC[EFFECT_MAGIC_SIZE] = {
  'L', 'U', 'E', 'F', 'O', 'B', 'S', '\0'
};
...
{
  if (offset == 0 || value == 0 || *offset > bytes.size() ||
      bytes.size() - *offset < 4) {
    return false;
  }
  const Byte* data = bytes.data() + *offset;
  *value = static_cast<U32>(data[0]) |
...
{
  U32 size = 0;
  if (text == 0 || !read_u32(bytes, offset, &size) ||
      size > MAX_NAME_SIZE || *offset > bytes.size() ||
...
{
  U32 size = 0;
  if (output == 0 || !read_u32(bytes, offset, &size) ||
      *offset > bytes.size() ||
      static_cast<Size>(size) > bytes.size() - *offset) {
    return false;
  }
                      ...
                      EffectObservation* observation)
{
  ...
  return store != 0 && observation != 0 && store->get(id, &texel) &&
         ...
         texel.get_output("observation", &output) &&
         output.type() == VALUE_BYTES && output.has_source() &&
         decode_effect_observation(output.source(), observation);
}
...
bool EffectIntent::set(const Byte* request_id, const char* operation,
                       const char* target, const Bytes& payload)
{
...
{
  if (!intent.valid() || value == 0 ||
      intent.operation().size() > UINT32_MAX ||
  ...
  bytes.insert(bytes.end(), intent.operation().begin(),
               intent.operation().end());
...
bool decode_effect_intent(const Value& value, EffectIntent* intent)
{
  ...
  const Size fixed_size = EFFECT_MAGIC_SIZE + 4 +
                          EffectIntent::REQUEST_ID_SIZE + 4 + 4 + 4;
  ...
  if (!read_string(bytes, &offset, &operation) ||
      !read_string(bytes, &offset, &target) ||
...
bool EffectObservation::set(const Byte* request_id, bool success,
                            const Bytes& bytes)
{
...
bool encode_effect_observation(const EffectObservation& observation,
                               Value* value)
{
  ...
  bytes.insert(bytes.end(), observation.bytes().begin(),
               observation.bytes().end());
  *value = Value(bytes);
  return true;
}

bool decode_effect_observation(const Value& value,
                               EffectObservation* observation)
{
  ...
  const Bytes& bytes = value.bytes();
  const Size fixed_size = EFFECT_MAGIC_SIZE + 4 +
                          EffectIntent::REQUEST_ID_SIZE + 1 + 4;
  if (bytes.size() < fixed_size ||
      memcmp(bytes.data(), OBSERVATION_MAGIC,
             sizeof(OBSERVATION_MAGIC)) != 0) {
    return false;
  }
  ...
  if (success > 1 || !read_bytes(bytes, &offset, &result) ||
      offset != bytes.size() ||
...
bool EffectExecutorRegistry::put(const char* operation,
...
bool EffectExecutorRegistry::get(const char* operation,
                                 EffectExecutor** executor) const
{
  if (operation == 0 || executor == 0) {
    return false;
  }
...
EffectBoundaryResult::EffectBoundaryResult()
    : boundary_code(EFFECT_INVALID_ARGUMENT),
...
{
  return boundary_code == EFFECT_PERFORMED ||
         boundary_code == EFFECT_REPLAYED;
}
...
bool make_effect_texel(const TexelId& id, const EffectIntent& intent,
                       Texel* texel)
{
  ...
  if (!made.put_output(output) ||
      !made.put_input(InputPort("capability", VALUE_BYTES))) {
    return false;
  }
  *texel = made;
  return true;
...
bool make_capability_texel(const TexelId& id,
                           const Capability& capability, Texel* texel)
{
...
EffectBoundary::EffectBoundary(
    Store* effect_store, Spool* effect_spool,
...
EffectBoundaryResult EffectBoundary::receipt(
    const EffectObservation& observation, EffectBoundaryCode code) const
{
  EffectBoundaryResult result;
  result.boundary_code = code;
...
{
  if (store == 0 || spool == 0 || authority == 0 || registry == 0 ||
      effect.is_unset() || !store->is_open()) {
  ...
  if (store->has(observation_id)) {
    ...
    if (!load_observation(store, observation_id, &observation) ||
      ...
      return error(EFFECT_MALFORMED_RECEIPT,
                   "stored effect receipt is malformed");
    }
    return receipt(observation, EFFECT_REPLAYED);
  }

  if (!capability_port.has_binding()) {
    return error(EFFECT_MISSING_CAPABILITY,
                 "effect capability is not connected");
  }
  const Fiber& binding = capability_port.binding();
  ValueOutcome capability_outcome;
  const Value* capability_value = 0;
  if (!spool->demand(binding.source(), binding.output().c_str(),
                     &capability_outcome) ||
      capability_outcome.status() == VALUE_UNAVAILABLE) {
    return error(EFFECT_MISSING_CAPABILITY,
                 "effect capability is unavailable");
  }
  if (!available_bytes(capability_outcome, &capability_value)) {
    return error(EFFECT_MALFORMED_CAPABILITY,
                 "effect capability demand failed");
  }
  Capability capability;
  if (!decode_capability(*capability_value, &capability)) {
    return error(EFFECT_MALFORMED_CAPABILITY,
                 "effect capability is malformed");
  }
  if (!authority->verify(capability, intent.operation().c_str(),
                         intent.target().c_str())) {
    return error(EFFECT_CAPABILITY_DENIED,
                 "effect capability does not grant intent");
  }

  EffectExecutor* executor = 0;
  if (!registry->get(intent.operation().c_str(), &executor)) {
    return error(EFFECT_MISSING_EXECUTOR,
                 "effect executor is not registered");
  }
  EffectObservation observation;
  ...
  if (!encode_effect_observation(observation, &observation_value) ||
    ...
    return error(EFFECT_INVALID_OBSERVATION,
                 "effect observation could not be encoded");
  }

  Transaction transaction;
  if (!store->begin(&transaction) ||
      !transaction.put(observation_texel) || !transaction.commit()) {
    if (transaction.is_active()) {
      transaction.abort();
    }
    return error(EFFECT_STORE_FAILED,
                 "effect observation could not be persisted");
  }
