...
String error_text(const char* message, const TexelId& texel,
                  const String& output)
{
...
Spool::Spool(const Store* fabric_store,
             const EvaluatorRegistry* evaluator_registry)
    : store(fabric_store),
...
bool Spool::demand(const TexelId& texel, const char* output,
...
Spool::DemandResult Spool::demand_internal(const TexelId& texel,
                                           const String& output,
                                           U64 generation)
{
  Endpoint endpoint;
  ...
  if (source_found != source_cache.end() &&
    ...
    result.effective_revision =
        source_found->second.output.effective_revision;
    return result;
  }
  ...
  if (!current.get_output(output.c_str(), &declared_output)) {
    ...
    result.outcome = ValueOutcome::error(
        error_text("missing output ", texel, output).c_str());
    result.effective_revision = 0;
    return result;
  }

  if (current.evaluator().empty() ||
      temporal_evaluator(current.evaluator())) {
...
Spool::DemandResult Spool::demand_source(const Texel& texel,
                                         ...
                                         U64 generation)
{
  ...
  if (found != source_cache.end() &&
      found->second.texel_revision == texel.revision() &&
  ...
  if (output.has_source()) {
    if (output.source().type() != output.type()) {
      outcome = ValueOutcome::error(
          error_text("source type mismatch at ", texel.id(),
                     output.name()).c_str());
    } else {
  ...
  if (!next_effective_revision(&effective_revision)) {
    DemandResult result;
    result.outcome =
        ValueOutcome::error("spool effective revisions exhausted");
    result.effective_revision = 0;
    return result;
  }
...
Spool::DemandResult Spool::demand_computed(const Texel& texel,
                                           const String& output,
                                           U64 generation)
{
  ...
  for (Size i = 0; i < texel.input_size(); ++i) {
    ...
    if (!texel.input_at(i, &input)) {
      return cache_error(texel, output, generation, revisions,
                         "invalid input table");
    }
    ...
    if (source_output.type() != input.type()) {
      String message = "input type mismatch at " +
                       endpoint_text(texel.id(), input.name());
      return cache_error(texel, output, generation, revisions, message);
    }

    DemandResult upstream =
        demand_internal(binding.source(), binding.output(), generation);
    inputs[input.name()] = upstream.outcome;
    revisions.push_back(upstream.effective_revision);
    if (upstream.outcome.status() == VALUE_ERROR) {
      return cache_error(texel, output, generation, revisions,
                         upstream.outcome.message());
    }
  }

  ComputeCache::iterator found = compute_cache.find(texel.id());
  if (found != compute_cache.end() &&
      found->second.texel_revision == texel.revision() &&
      same_revisions(found->second.input_revisions, revisions)) {
    CachedOutputMap::const_iterator cached =
        found->second.outputs.find(output);
...
Spool::DemandResult Spool::evaluate(const Texel& texel, const String& output,
                                    U64 generation,
                                    const ValueOutcomeMap& inputs,
                                    const RevisionList& revisions)
{
  Evaluator* evaluator = 0;
  if (registry == 0 ||
      !registry->get(texel.evaluator().c_str(), &evaluator)) {
  ...
  for (; returned != evaluated.end(); ++returned) {
    ...
    if (returned->second.status() == VALUE_AVAILABLE &&
      ...
      String message = "evaluator output type mismatch at " +
                       endpoint_text(texel.id(), returned->first);
      return cache_error(texel, output, generation, revisions, message);
    }
  }
...
Spool::DemandResult Spool::cache_error(const Texel& texel,
                                       const String& output, U64 generation,
                                       const RevisionList& revisions,
