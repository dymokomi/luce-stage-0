...
const Byte CAPABILITY_MAGIC[CAPABILITY_MAGIC_SIZE] = {
  'L', 'U', 'C', 'A', 'P', '\0', '\0', '\0'
};
...
{
  return String(reinterpret_cast<const char*>(token),
...
bool Authority::issue(const char* operation, const char* scope,
                      Capability* capability)
{
                       ...
                       const char* scope) const
{
  if (!capability.valid() || operation == 0 || scope == 0 ||
      capability.allowed_operation != operation ||
      capability.allowed_scope != scope) {
    return false;
  }

  CapabilityGrantTable::const_iterator found =
      grants.find(token_key(capability.token));
...
bool encode_capability(const Capability& capability, Value* value)
{
  ...
  bytes.insert(bytes.end(), capability.token,
               capability.token + Capability::TOKEN_SIZE);
...
bool decode_capability(const Value& value, Capability* capability)
{
  ...
  const Size fixed_size = CAPABILITY_MAGIC_SIZE + 4 +
                          Capability::TOKEN_SIZE + 4 + 4;
  ...
  if (!read_string(bytes, &offset, &decoded.allowed_operation) ||
      !read_string(bytes, &offset, &decoded.allowed_scope) ||
      offset != bytes.size() || !decoded.valid()) {
    return false;
  }
  *capability = decoded;
  return true;
