...
bool make_view(const TexelId& id, const char* evaluator,
               const Strings& inputs, Texel* view)
{
  ...
  for (Size i = 0; i < inputs.size(); ++i) {
    if (inputs[i].empty() ||
        !result.put_input(InputPort(inputs[i].c_str(), VALUE_TEXT))) {
      return false;
    }
  }
  if (!result.valid()) {
...
void ProseViewEvaluator::evaluate(const Texel&,
                                  ...
                                  ValueOutcomeMap* outputs)
{
  ...
  (*outputs)[VIEW_INTERFACE_OUTPUT] =
      ValueOutcome::available(Value(rendered));
}

void TableViewEvaluator::evaluate(const Texel&,
                                  const ValueOutcomeMap& inputs,
                                  ValueOutcomeMap* outputs)
{
  if (outputs == 0) {
  ...
  const String border =
      "+" + repeated('-', name_width + 2) +
      "+" + repeated('-', value_width + 2) + "+\n";
  String rendered = border;
  rendered += "| Field" + repeated(' ', name_width - 5) +
              " | Value" + repeated(' ', value_width - 5) + " |\n";
  rendered += border;
  ...
  for (; input != inputs.end(); ++input) {
    ...
    rendered += "| " + input->first +
                repeated(' ', name_width - input->first.size()) + " | " +
