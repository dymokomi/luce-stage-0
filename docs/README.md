...
| [PACKAGES.md](PACKAGES.md) | **Proposed, not built.** The consuming half of packages: `luce.zon` roots and exact-version wants, dotted imports with `as`, probe-every-tier resolution that refuses ambiguity instead of ordering it, the `.luce/` store, and the four seam changes package isolation costs the loader. |
