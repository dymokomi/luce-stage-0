...
| [FILESYSTEM.md](FILESYSTEM.md) | **Proposed.** The filesystem surface: `paths` is the name and `files` is the world, because asking is touching; no `Path` value type; and `files.kind(p) -> Kind?!` as the one question, answering absence and refusal separately, subsuming `exists`, with listings that carry kinds. |
