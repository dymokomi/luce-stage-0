...
| [PACKAGES.md](PACKAGES.md) | **Proposed, not built.** The consuming half of packages: `luce.yaml` roots (a strict YAML subset) and exact-version wants, dotted imports with `as`, probe-every-tier resolution that refuses ambiguity instead of ordering it, the `.luce/` store, and the four seam changes package isolation costs the loader. |
| [BINDING.md](BINDING.md) | **Ratified, not built.** Bound methods — the method travels with its struct: `receiver.method` as a function value whose environment is the receiver, value receivers copied at the bind, carrying receivers borrowing or moving by the verbs S21/S24 already speak, `func(...)?` as the storable form, and anonymous captures refused permanently. Closures with a nominal environment and no new ownership rule. |
| [TESTING.md](TESTING.md) | **Proposed, not built.** `luce test`: `func test_*()` in `tests/`, discovered by name and refused-not-skipped when malformed, run one `luce_main` call per test through a synthesized entry in a blessed shape — the CLI drives, so a trap fails the test and never the run. |
