...
state, **nothing new to reason about at all**.  An inline function
that reaches a variable from its enclosing scope — a *closure* —
carries an environment the reader cannot see on the page.  Luce's
honest spelling of behavior-plus-state already exists: **a struct (or
`class`) with a method**, the state explicit, named, and visible.
...
tasks.append(spawn crunch(chunk))          # threads: named functions already serve
...
| **D3** | Function values have no equality or ordering, and `string(f)` answers the function's name.  They copy freely. |
...
| **D5** | A function value is passable like any other; the *call through the value* type-checks its arguments exactly as a direct call does. |
...
design and its *As built* section is what shipped.  Capturing closures
are the ARC model's (docs/MEMORY.md) and are not yet built; the lambda
stays capture-free until they are.

D2's "a function value is an index" is likewise one version old: it is
now the pair `{function, receiver}`, carried as a two-slot run.  A
bound method holds a **reference** to its receiver, which ARC keeps
alive for as long as the value lives (docs/BINDING.md, third *As
built*).  That is the one thing the runtime learns about function
values, and it is what makes a stored bound method safe.
...
rest of D3 stands: function values copy freely.
...
value may hold a reference to a receiver, and reference types do not
cross a worker boundary (docs/MEMORY.md D6) — so a function value is
refused as a spawned function's parameter and as its result.  A call
through a value still type-checks its arguments exactly as a direct
call does, which is what D5 was really about.
...
the arity and the argument types, a function type has no parameter
names and no defaults, so a named argument is refused where it is
written.  How each argument is passed is the signature's business and
the callee's spelling changes nothing about it (D5): a call suffix
type-checks its arguments exactly as a name does.
