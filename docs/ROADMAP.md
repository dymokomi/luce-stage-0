...
| Toolchain and release | Complete for the current macOS release | `luce` builds, checks, prints IR, tests, and maintains local packages; the installer supplies the compiler, editor, runtime libraries, TermUI, and VS Code extension. | The published one-command release is macOS ARM64 only. Luce remains pre-1.0 and promises no source compatibility between 0.x releases. |
