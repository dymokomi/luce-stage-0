...
| Toolchain and release | Implemented | `luce` builds, checks, prints IR, tests, and maintains local packages; no-argument help pins that full command surface, and the test runner flushes per-test progress before entry. One checked installer supplies the compiler, editor, runtime libraries, TermUI, and VS Code extension on macOS 15+ ARM64 and glibc Linux ARM64/x86-64. Linux releases contain pinned static LLVM. | Windows and musl Linux are not published. Luce remains pre-1.0 and promises no source compatibility between 0.x releases. |
