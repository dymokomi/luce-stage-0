...
| **D9** | **`with` is unnecessary, not missing, and a Python reader is told so once, plainly.**  `var f = try open(p)` — the binding owns the file, the end of its scope closes it, `free f` closes it early, `give f`/`return f` move it, and using one after it is closed traps like any use after free (`docs/OWNERSHIP.md`, unchanged).  Scope ownership is `with` without the block, applied to *every* owned thing rather than to the ones a library remembered to give an `__exit__`; a Luce program cannot leak a file by forgetting a keyword, because there is no keyword to forget.  **`close()` is not offered**: it would have to poison the receiver exactly as `free` does, so it would be `free` under a second name for one concept (§62), and an *idempotent* `close()` would need a "closed but not poisoned" state — a second lifetime story beside scope ownership and the one state §8 says the type must never hold.  The existing diagnostic already teaches half of it (*"file has no method close; free f closes it"*) and gains the other half: *"…and the end of the owning scope closes it."*  This sentence belongs in `docs/STD.md`, on luce.luciaos.com, and in the diagnostic — three places, because a Python programmer will look in all three. |
...
no way for it to forget: the binding owns the file and a scope end
releases what it owns, the same sentence that frees a list.  Python's
...
| `with open(p) as f:` | scope ownership already closes it — Luce being better, not shorter | `var f = try open(p)` |
| `f.close()` | it would be `free f` under a second name (§62) | `free f`, or let the scope end |
...
| **R2** | **Does `close()` exist?**  Python programmers will type it.  Options: refused, with the diagnostic teaching `free f` (recommended); an alias for `free f`; or an idempotent no-op-if-closed, which is Python's actual behaviour. | **Refused** (D9).  A working `close()` must poison the receiver exactly as `free` does, making it `free` under a second name for one concept (§62); an idempotent one needs a "closed but not poisoned" state, a second lifetime story beside scope ownership and the one state §8 says the type must never hold.  The sentence a Python reader needs is one sentence and it goes in three places: the diagnostic, `docs/STD.md`, and loom's own documentation. |
