...
**`bench/`** — paired C/Luce benchmarks with identical algorithms and outputs (`bench/run.sh` compiles, cross-checks, and times them; snapshot and workflow in `docs/BENCHMARKS.md`; the full speed decision record — every option weighed from the first benchmark to the native engine — in `docs/SPEED.md`). Run it before and after engine/lowering/std changes — the native/C and interp/C ratios are the regression guard (native currently 1.6-3x C on the covered benches). Benchmark only optimized interpreters (build.sh's default); a Debug build is 4-5x slower and its numbers are meaningless.
