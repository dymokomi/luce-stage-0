...
```sh
./build.sh         # zig build --prefix build -Doptimize=ReleaseSafe: installs build/luce, build/loom, build/programs/*.lc
...
- `luce` (src/apps/luce/) — the compiler: `luce build FILE.luc [-o FILE.lc] [--release]`, `luce check`, `luce ir`. Debug builds (default) embed trap locations; `--release` strips them, changing nothing else (`docs/MODES.md`).
...
**The standard library** (`src/luce/std/*.luc`, table in `compile.zig`, docs in `docs/STD.md`): ordinary Luce source embedded in the compiler via `@embedFile`, resolved before the import loader — std names (`math`, `strings`, `files`) are reserved and work everywhere the compiler does, with no install path. Std obeys every language rule including the host gate. Proven by `src/luce/std_spec.zig` (pure modules) plus hosted tests beside `TestHost`. A new module is: the `.luc` file, one `std_modules` row, tests, an STD.md section.
...
**`bench/`** — paired C/Luce benchmarks with identical algorithms and outputs (`bench/run.sh` compiles, cross-checks, and times them; snapshot and workflow in `docs/BENCHMARKS.md`; the full speed decision record — every option weighed from the first benchmark to the native engine — in `docs/SPEED.md`). The authoritative regression check is `bench/compare.sh GIT-REF` — a same-host interleaved A/B of the working tree against any commit (absolute times never compare across hosts; this container's host changes between sessions). Run it before and after engine/lowering/std changes; native currently ~1.1-1.2x C on scalar benches, ~1.8x on the vectorizable arrays bench, ~3.6x on allocation-heavy strings (milestone-4 snapshot in BENCHMARKS.md). Benchmark only optimized interpreters (build.sh's default); a Debug build is 4-5x slower and its numbers are meaningless.
