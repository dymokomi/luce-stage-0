...
**`bench/`** — paired C/Luce benchmarks with identical algorithms and outputs (`bench/run.sh` compiles, cross-checks, and times them; snapshot and workflow in `docs/BENCHMARKS.md`). Run it before and after interpreter/lowering/std changes — the Luce/C ratio is the regression guard. Benchmark only optimized interpreters (build.sh's default); a Debug build is 4-5x slower and its numbers are meaningless.
